// designated initialization in structure 
#include<stdio.h>

struct question{
	int a;
    float b;
    int c;
};

void main(){
   struct question q = {.c = 23, .a = 6, .b = 7}; //designated initialization 
//	struct question q = {23, 6.5, 7};
    printf("a = %d\nb = %f\nc = %d", q.a , q.b, q.c); // a=6 b=7.000000 c 23
}







/*
// structure packing
// means it do not add padding bytes so memory wastage is less 
// but the performance is decreased 
#include<stdio.h>
#pragma pack(1) // with this it don't add padding bytes so memory is less required 
// #pragma pack(1) // if we write This then size is 6 byte 
// if we don't write #pragma pack(1) then compiler do automatically padding and size with 12 byte 
typedef struct student {
 	char a; // 1 byte
     int b; // 4 byte
     char c; // 1 byte 
 } hello ;
void main(){
	hello s;
    printf("size of s = %d byte", sizeof(s)); // 6 byte
}


/*
// structure packing 
#include<stdio.h>
// #pragma pack(1) // if we write This then size is 6 byte 
//if we don't write #pragma pack(1) then compiler do automatically padding and size with 8 byte 
 struct student {
 	char a;
     char b;
     int c;
 };
void main(){
	struct student s;
    printf("size of s = %d byte ", sizeof(s)); // 6 byte
}




/*
// Structure padding 
// memory allocation
// little bit lengthy topic so understanding topic on your note book 
// lot of theory is here in structure padding 
#include<stdio.h>
typedef struct student {
 	char a;
     int b;
     char c;
 } hello ;
void main(){
	hello s;
    printf("size of s = %d byte", sizeof(s)); // 12 byte
}


/*
// structure padding 
#include<stdio.h>
 struct student {
 	char a;
     char b;
     int c;
 };
void main(){
	struct student s;
    printf("size of s = %d byte ", sizeof(s)); // 8 byte
}
*/







/*
// typedef in structure
// with the help of typedef we can create our own name or syntonym  of any datatype name 
// example jayanti == jenny
// hum koi bhi bna skta hai jisse code ka size chota ho jata hai 
#include<stdio.h>

typedef struct student{ // typedef is used to make syntonym 
	int rollno;
    char name[20];
    float marks;
 
} Aditya;
// Aditya == struct student 
void main(){
	Aditya s = {1, "jenny", 7.4}; // s is object 
    printf("%d\n", s.rollno); // 1
    printf("%s\n", s.name); // jenny
    printf("%.2f\n", s.marks); // 7.40
}
*/





/*
// union are like structure
// but the memory allocation is different 
// advantage of union - less usage of memory
// drawback - it hold only last entered value 
// jo jo humne structure ke sath kiya vo sbb hum union ke sath bhi kar skte hai 
// we can use pointer also in union 
#include<stdio.h>
union value{
	int a;
    char b;
    float c;
};
void main(){
/*
// if you initializa like this then it took first value 
union value s = {7, 67 , 56.6};
    
    printf("%d\n", s.a); // give 7
    printf("%c\n", s.b); // give garbage value
    printf("%f\n", s.c); // 0.000000
*/    

/*
// union initialization syntex
	union value s;
    s.a = 5;
    s.b = 8;
    s.c = 7.9;
    
    printf("a = %d\n", s.a); // give garbage value
    printf("b = %c\n", s.b); // give garbage value
    printf("c = %f\n", s.c) ; // 7.900000
    
    union value *ptr = &s; // pointer to storing a address 
    printf("c = %f", ptr->c); // 7.900000
 
}
*/



/*
// pointer to structure or structure pointer 
#include<stdio.h>
struct student{
    int uid;
    char name[20];
    float cgpa;
};

void main(){
	struct student s = {2, "aditya", 7.45};
    struct student *ptr;
    ptr = &s;
    printf("%d\n", ptr->uid); // 2
    printf("%d\n", s.uid); // 2
    printf("%d\n", (*ptr).uid); // 2 
    printf("%s\n", (*ptr).name);  // Aditya 
   printf("%f", (*ptr).cgpa); // 7.450000
    
}
*/



/*
// Array of structure using for loop 
#include<stdio.h>
struct student{
	int rollno;
    char name[20];
    float marks;
};

void main(){
//	struct student s[0];
    struct student s[3];
    int i = 0;
    for(i = 0; i<3; i++){
    	printf("enter the details of s1 : "); // index is zero 0
        scanf("%d %s %f", &s[i].rollno, &s[i].name, &s[i].marks );
    }
    // for printing structure value 
    for(i =0; i<3; i++){
    	printf("rollno = %d\n name = %s\n marks = %f\n\n", s[i].rollno, &s[i].name, s[i].marks );
    }
}

*/



/*
// Array of structure
#include<stdio.h>
struct student{
	int rollno;
    char name[20];
    float marks;
};

void main(){
//	struct student s[0];
    struct student s[3]; // it count 3 values s1,s2,s3
// it is like array so index value is start from zero 
// for first element of s1    
    printf("enter the details of s1 : "); // index is zero 0
    scanf("%d %s %f", &s[0].rollno, &s[0].name, &s[0].marks );
    printf("rollno = %d name = %s marks = %f\n\n", s[0].rollno, &s[0].name, s[0].marks );
    
    printf("enter the details of s2 : "); // index is 1
    scanf("%d %s %f", &s[1].rollno, &s[1].name, &s[1].marks );
    printf("rollno = %d name = %s marks = %f\n\n", s[1].rollno, &s[1].name, s[1].marks );
    
    printf("enter the details of s3 : "); // index is 2
    scanf("%d %s %f", &s[2].rollno, &s[2].name, &s[2].marks );
    printf("rollno = %d name = %s marks = %f\n\n", s[2].rollno, &s[2].name, s[2].marks );
    
}
*/



/*
// initialization of structure 
#include<stdio.h>
struct student { 
    int rollno ;
    char name[20];
    float marks;
 };
//  } s = {1, "Aditya", 67.24} ; // it also fine you also intilize like this 
//    s2 = {1, "Aditya", 67.24} ;  // two s and s2 are wrong this is not allowed 

int main(){
//	struct student s = {1, "jenny", 6.24}; // initialization, order should same
//    struct student s = {"hshs", 2, 6.2}; // this is wrong order is not match 
//      struct student s = {1}; // it is partially initialized  rest of all are become null char ,float = null
    struct student s1 = {1, "Aditya", 67.24} ; // ese jine marzi variables initialization karlo
      struct student s2 = {10, "Kumar", 9.5} ; // 
}
*/






/*
// declarating variable in structure
#include<stdio.h>

struct student { 
    int rollno ;
// 	int rollno = 23; we can't intialize rollno bcs memory has not been allocated 
    char name[20];
    float marks;
//    int marks; // it give error bcs we can't use two same names of variables 
};
 //} s1, s2;

int main(){
//   struct student s3; // it also working 
    struct student s1, s2, s3; // s1 s2 S3 are variable or object 
    printf("%d\n", sizeof(s1)); // give 28 bcs it calculat int 4 , char[20] 20, float 4 
    printf("%d", sizeof(struct student)); // 28
}
*/

/*
int main(){ 
    struct student a;
    printf("%d\n", sizeof(a)); // give 28 bcs it calculat int 4 , char[20] 20, float 4 
    printf("%d", sizeof(struct student)); // 28
}








/* 
// Structure beginning
// important about structure
// two variables have can't not same name 
// we can't initialize variables in structure 
#include<stdio.h>

struct student{ 
    int rollno ;
// 	int rollno = 23; we can't intialize rollno bcs memory has not been allocated 
    char name[20];
    float marks;
//    int marks; // it give error bcs we can't use two same names of variables 
};

int main(){ 
    struct student a;
    printf("%d", sizeof(a)); // give 28 bcs it calculat int 4 , char[20] 20, float 4 
}

*/
    